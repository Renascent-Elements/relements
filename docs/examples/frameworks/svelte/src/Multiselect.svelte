<script>
  ...
  onMount(() => {
    ...
    // node Svelte doesn't manage — so it survives re-renders.
    const controller = enhanceMultiSelect(wrap);
...
<section aria-labelledby="ms-h">
  ...
  <!-- Above the field so the open (absolute) panel can't cover it; bumping
       `renders` re-renders without unmounting. -->
  <p>
    <button id="ms-rerender" type="button" onclick={() => renders++}>Re-render</button>
    <output id="ms-renders">{renders}</output> renders
  </p>
  <div class="re-field">
    <span class="re-field__label" id="ms-label">Frameworks</span>
    <div id="ms-wrap" bind:this={wrap}>
