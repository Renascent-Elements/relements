...
Relements targets modern evergreen browsers. Support is **two-tier**, matching the HTML-first philosophy (see `HTML_FIRST_POLICY.md`): a broad semantic HTML + CSS baseline, plus progressive enhancements that need recent engines and degrade gracefully where they are unavailable.
...
- `HTML_FIRST_POLICY.md` — the graceful-degradation ethos.
- `VERSIONING.md` — dropping a supported tier is a breaking change.
