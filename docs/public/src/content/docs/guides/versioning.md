---
title: Versioning
description: Semantic versioning and what counts as a breaking change.
---

Relements follows [Semantic Versioning](https://semver.org) (`MAJOR.MINOR.PATCH`). This document defines the **public API surface** the contract applies to and what counts as a breaking change. It is the authoritative policy; the summary in [`TECHNICAL_DOCUMENTATION.md`](https://github.com/Renascent-Elements/relements/blob/main/docs/TECHNICAL_DOCUMENTATION.md) points here.
...
- Dropping a supported browser tier (see [Browser support](/relements/guides/browser-support/)).
...
- Docs updated (including [Browser support](/relements/guides/browser-support/) if support changed).
- A changeset added, at the bump level this policy dictates.

## Release mechanics

Releases are automated with [Changesets](https://github.com/changesets/changesets) — see the **Releasing** section of the [README](https://github.com/Renascent-Elements/relements/blob/main/README.md). Add a changeset (`pnpm changeset`) choosing the bump level per this policy; merging to `main` opens a version PR; merging that publishes to npm.
