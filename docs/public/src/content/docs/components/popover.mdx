...
Uses the native `popover` attribute. The browser handles light dismiss and focus; the `enhancePopover` behavior anchors position and dispatches `re-toggle`. Prefer a self-managing tag? The [`<re-popover>`](/relements/custom-elements/re-popover/) custom element wraps this same pattern.
