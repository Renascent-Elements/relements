...
- **Editor IntelliSense is generated — regenerate, don't hand-edit.** `packages/core/{html,css}.custom-data.json` + `web-types.json` are produced by `scripts/gen-editor-data.mjs` (run `pnpm -F @relements/core gen:editor-data`). It _derives_ the tags, `data-*` attributes/values, `data-re-*` hooks, and `--re-*` tokens from the component CSS / behavior JS / `tokens.css`, so new ones appear automatically after a regen — only the hover prose and the per-element list are curated in that script. `tests/unit/editor-data.spec.ts` fails CI if the committed files are stale or if a new `src/elements/*.js` lacks a curated entry, so just follow the test message. (`prepublishOnly` also regenerates, so a publish can't ship stale data.)
